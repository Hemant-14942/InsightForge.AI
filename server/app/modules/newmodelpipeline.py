...
class EnhancedMLPipeline:
    ...
    def train_and_evaluate(self, df: pd.DataFrame, task_type: str, target_col: str) -> Dict[str, Any]:
        ...
        print(f"Target column received --------->>><<<<-------------: {target_col}")

        # ✅ Step 1: Normalize and resolve the actual target column name
        # Ensure consistent cleaning of column names
        def _normalize_column(col: str) -> str:
            return col.strip().lower().replace(" ", "_")

        # Step 1: Normalize and resolve the actual target column name
        print(f"Target column received --------->>><<<<-------------: {target_col}")
        target_col_clean = _normalize_column(target_col)
        print(f"Target column cleaned --------->>><<<<-------------: {target_col_clean}")

        # Map normalized column names to original ones in df
        columns_map = {_normalize_column(col): col for col in df.columns}
        print(f"Columns map --------->>><<<<-------------: {columns_map}")

        actual_target_col = columns_map.get(target_col_clean)
        print(f"Actual target column: {actual_target_col}")

        if not actual_target_col:
            logging.error(f"❌ Target column '{target_col}' not found in dataset!")
            raise ValueError(f"Target column '{target_col}' not found")

        logging.info(f"✅ Matched actual column name: {actual_target_col}")
        ...
        # ✅ Step 4: Handle class imbalance if classification
